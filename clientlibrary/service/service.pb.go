...
func PluginExample(b []byte) (resp []byte) {
	defer func() {
		if PanicHandler != nil {
			if r := recover(); r != nil {
				resp, _ = (&pb.RpcPluginExampleResponse{Error: &pb.RpcPluginExampleResponseError{Code: pb.RpcPluginExampleResponseError_UNKNOWN_ERROR, Description: "panic recovered"}}).Marshal()
				PanicHandler(r)
			}
		}
	}()
	...
	if err := in.Unmarshal(b); err != nil {
		resp, _ = (&pb.RpcPluginExampleResponse{Error: &pb.RpcPluginExampleResponseError{Code: pb.RpcPluginExampleResponseError_BAD_INPUT, Description: err.Error()}}).Marshal()
		return resp
	}

	resp, _ = clientCommandsHandler.PluginExample(context.Background(), in).Marshal()
	return resp
}
...
func (h *ClientCommandsHandlerProxy) PluginExample(ctx context.Context, req *pb.RpcPluginExampleRequest) *pb.RpcPluginExampleResponse {
	actualCall := func(ctx context.Context, req any) (any, error) {
		return h.client.PluginExample(ctx, req.(*pb.RpcPluginExampleRequest)), nil
	}
	for _, interceptor := range h.interceptors {
		toCall := actualCall
		currentInterceptor := interceptor
		actualCall = func(ctx context.Context, req any) (any, error) {
			return currentInterceptor(ctx, req, "PluginExample", toCall)
		}
	}
	call, _ := actualCall(ctx, req)
	return call.(*pb.RpcPluginExampleResponse)
}
